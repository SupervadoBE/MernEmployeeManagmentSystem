...
export const inngest = new Inngest({ id: "employee-managment-system" })
...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if(!attendance?.checkOut){
        ...
        await step.run("update-attendance-late", async () => {
            const currentAttendance = await Attendance.findById(attendanceId)
            if(!currentAttendance?.checkOut){
                currentAttendance.checkOut = new Date(attendance.checkIn).getTime() + 4 * 60 * 60 * 1000
...
const leaveApplicationReminder = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Wait for 24 hours
    await step.sleep("wait-24-hours", "1d")

    // get Leave Application data
    let leaveApplication = await step.run("get-leaveApplication", async () => {
...
const attendanceReminderCron = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const today = await step.run("get-today-date", () => {
        const startUTC = new Date(new Date().toLocaleDateString("en-CA", {timeZone: "Europe/Istanbul"}) + "T00:00:00+03:00")
            ...
            const emailPromises = absentEmployees.map((emp) => {
                ...
                sendEmail({
                    ...
                    body: ` <div style="max-width: 600px; font-family: Arial, sans-serif">
                                ...
                                <p style="font-size: 16px">We notice you haven't marked your attendance yet today.</p>
