...
const ProfileForm = ({initialData, onSuccess}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.post("/profile", fromData)
            ...
            <div className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="name" disabled value={`${initialData.firstName} ${initialData.lastName}`} className="bg-slate-50 text-slate-400 cursor-not-allowed" />
                    </div>

                    <div>
                        <label htmlFor="email" className="block text-sm font-medium text-slate-700 mb-2">Email</label>
                        <input type="email" id="email" disabled value={`${initialData.email}`} className="bg-slate-50 text-slate-400 cursor-not-allowed" />
                    </div>

                    <div className="sm:col-span-2">
                        <label htmlFor="position" className="block text-sm font-medium text-slate-700 mb-2">Position</label>
                        <input type="text" id="position" disabled value={`${initialData.position}`} className="bg-slate-50 text-slate-400 cursor-not-allowed" />
                    </div>
                </div>
                <div>
                    <label htmlFor="bio" className="block text-sm font-medium text-slate-700 mb-2">Bio</label>
                    <textarea disabled={initialData.isDeleted} name="bio" id="bio" defaultValue={initialData.bio || ""} placeholder="Write a brief bio..." className={`resize-none ${initialData.isDeleted ? "bg-slate-50 text-slate-400 cursor-not-allowed" : ""}`} />
