...
const CheckInButton = ({todayRecord, onAction}) => {
    ...
    if(todayRecord?.checkOut) {
        return (
            <div className="flex clex-col items-center justify-center p-8 bg-slate-50 rounded-2xl border border-slate-200">
    ...
    return (
        <div className="absolute bottom-4 right-4 flex flex-col z-1">
            <button onClick={handleAttendance} disabled={loading} className={`w-full max-w-xs flex justify-between items-center gap-8 p-4 rounded-xl bg-linear-to-br text-white ${isCheckedIn ? "from-slate-700 to to-slate-900" : "from-indigo-600 to-indigo-700"}`}>
